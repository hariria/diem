    ...
    fn run_prologue<S: MoveResolver>(
        ...
        match transaction.payload() {
            ...
            TransactionPayload::ModuleBundle(_module) => {
                self.0.check_gas(&txn_data, log_context)?;
                let mut module_sha3 = diem_crypto::hash::DefaultHasher::new(&[]);
                for sub_module in _module.iter() {
                    module_sha3.update(sub_module.code());
                }
                let final_module_sha3 = module_sha3.finish();
                self.0
                    .run_module_prologue(session, &txn_data, &currency_code, log_context, final_module_sha3.to_vec())
            }
