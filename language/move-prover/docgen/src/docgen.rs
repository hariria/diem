...
pub struct DocgenOptions {
    ...
    /// If this is being compiled relative to a different place where it will be stored.
    pub compile_relative_to: bool,
}
...
impl<'env> Docgen<'env> {
    ...
    fn make_file_in_out_dir(&self, name: &str) -> String {
        if self.options.compile_relative_to {
            name.to_string()
        } else {
